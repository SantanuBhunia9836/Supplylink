...
const LoginPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (validateForm()) {
            try {
                // The login function from AuthContext now returns the user's data
                const user = await login(formData);
                toast.success("Login successful!");

                // Check the user's status and navigate to the correct page
                if (user && user.is_seller) {
                    navigate('/dashboard'); // Sellers are sent to the dashboard
                } else {
                    navigate('/'); // Non-seller vendors are sent to the homepage
                }
            } catch (err) {
