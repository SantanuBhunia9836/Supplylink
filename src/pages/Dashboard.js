...
const Dashboard = () => {
  ...
  const renderPage = () => {
    ...
    if (user.is_seller) {
      ...
      // This is the component that would render the different forms
      // for filling in details if they are missing.
  ...
  const mainContentClass = user.is_seller
  ...
  return (
    <div className="flex h-screen bg-gray-100 font-sans">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className={mainContentClass}>{renderPage()}</main>
      </div>
    </div>
  );
};
