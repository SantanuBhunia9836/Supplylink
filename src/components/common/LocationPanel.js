...
const LocationPanel = ({ isOpen, onClose, onSuccess }) => {
  ...
  // --- FIX: Get user and context functions ---
  ...
  // --- FIX: Implemented the new save logic ---
  const handleSubmit = async (e) => {
    ...
    try {
      // If the user is logged in, check their profile for an existing location.
      if (user) {
        const vendorProfile = await getVendorProfile();
        // Check if location data is missing from the profile.
        const hasLocation =
          vendorProfile && vendorProfile.latitude && vendorProfile.longitude;

        // If the logged-in user does NOT have a location, create one in the database.
        if (!hasLocation) {
          await apiCreateVendorLocation(formData);
        }
      }

      // For ALL users (logged-in or guest), update the location in the global context.
      // This step triggers the seller search to re-run on the landing page.
      await updateLocation({
        latitude: formData.latitude,
        longitude: formData.longitude,
      });

      // Call the onSuccess prop (passed from App.js) to close the modal.
      if (onSuccess) {
        onSuccess();
      }
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4 transition-opacity duration-300">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
        <div className="p-6 border-b flex justify-between items-center">
          ...
          <button
            onClick={onClose}
            className="text-gray-400 hover:text-gray-800 text-3xl"
          >
            &times;
          </button>
        </div>
        ...
        <div className="p-6 overflow-y-auto">
          <div
            className={`grid gap-8 transition-all duration-500 ease-in-out ${
              showMap ? "md:grid-cols-2" : "md:grid-cols-1"
            }`}
          >
            <div
              className={`space-y-4 transition-transform duration-500 ${
                showMap ? "" : "md:col-span-1"
              }`}
            >
