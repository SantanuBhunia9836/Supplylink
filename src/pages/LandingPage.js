...
// --- Modern Loading Spinner for Popup ---
                ...
                {step === "email" ? (
                    <form onSubmit={handleEmailSubmit}>
                        <div className="mb-4">
                            ...
                            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="user@example.com" className="w-full px-4 py-3 border border-gray-300 rounded-lg" autoFocus />
                        </div>
                        {error && <p className="text-red-600 text-sm mb-4 text-center">{error}</p>}
                        <button type="submit" className="w-full bg-blue-600 text-white py-3 rounded-lg font-semibold hover:bg-blue-700">Continue</button>
                        <div className="mt-4 text-center">
                            <p className="text-sm text-gray-600">
                                Don't have an account?{" "}
                                <button type="button" onClick={() => { onClose(); navigate("/register"); }} className="text-blue-600 hover:underline font-medium">Sign up</button>
                            </p>
                        </div>
                    </form>
                ) : (
                    <form onSubmit={handlePasswordSubmit}>
                        <div className="mb-4">
                            ...
                            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full px-4 py-3 border border-gray-300 rounded-lg" autoFocus />
                        </div>
                        {error && <p className="text-red-600 text-sm mb-4 text-center">{error}</p>}
                        <button type="submit" disabled={isLoading} className="w-full bg-blue-600 text-white py-3 rounded-lg font-semibold hover:bg-blue-700 flex justify-center items-center">
                            {isLoading ? <LoadingSpinner /> : 'Login'}
                        </button>
                        <div className="mt-4 text-center">
                            <button type="button" onClick={() => setStep("email")} className="text-sm text-gray-500 hover:text-gray-700">← Back to email</button>
                        </div>
                    </form>
                )}
            </div>
      ...
      <header className="relative bg-gradient-to-br from-blue-50 via-purple-50 to-indigo-50">
        <div className="container mx-auto px-6 py-24 md:py-32 text-center">
          ...
          <p className="text-lg md:text-xl text-gray-700 mt-8 max-w-3xl mx-auto">
            Streamline your procurement. Find the best local suppliers for your daily needs.
          </p>
        </div>
      </header>
        ...
        <div className="container mx-auto px-6 text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10">
            <div className="p-6">
              ...
              <p className="text-gray-600">Save hours every day by replacing multiple phone calls with a single order on our app.</p>
            </div>
            <div className="p-6">
              ...
              <p className="text-gray-600">We partner with trusted, high-quality local vendors to ensure you get the best products.</p>
            </div>
            <div className="p-6 sm:col-span-2 md:col-span-1">
              ...
              <p className="text-gray-600">Track your spending and order history with our easy-to-use dashboard analytics.</p>
            </div>
          </div>
        </div>
      </section>
