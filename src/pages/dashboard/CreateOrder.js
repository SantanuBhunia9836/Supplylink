        ...
        <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="flex-grow bg-white p-6 rounded-lg shadow">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    {products.map(product => (
                        <div key={product.id} className="border rounded-lg p-4 flex flex-col justify-between transition-shadow hover:shadow-md">
                            <div>
            ...
            <div className="w-full lg:w-80 flex-shrink-0 bg-white p-6 rounded-lg shadow self-start sticky top-6">
                 ...
                 <div className="space-y-3 text-gray-700 min-h-[150px]">
                    {orderItems.length === 0 ? (
                        <p className="text-gray-500 text-center pt-10">Your order is empty.</p>
                    ) : (
                        orderItems.map(item => (
                            <div key={item.id} className="flex justify-between items-center">
                                ...
                                <div className="flex items-center space-x-2">
                                    <button onClick={() => handleRemoveFromOrder(item.id)} className="w-6 h-6 bg-gray-200 rounded-full text-lg font-bold flex items-center justify-center">-</button>
                                    <span className="font-semibold w-4 text-center">{item.quantity}</span>
                                    <button onClick={() => handleAddToOrder(item)} className="w-6 h-6 bg-gray-200 rounded-full text-lg font-bold flex items-center justify-center">+</button>
                                </div>
                            </div>
                        ))
                    )}
